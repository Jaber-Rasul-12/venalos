+(function ($) {
    "use strict";
    if ($.wn.builder === undefined) $.wn.builder = {};
    ...
    DataRegistry.prototype.set = function (
        plugin,
        type,
        subtype,
        data,
    ...
    DataRegistry.prototype.isCacheObsolete = function (timestamp) {
        return Date.now() - timestamp > 60000 * 5;
    };
    DataRegistry.prototype.loadDataFromServer = function (
        $formElement,
        plugin,
        type,
        subtype,
        ...
        if (this.requestCache[cacheKey] === undefined) {
            this.requestCache[cacheKey] = $formElement
                .request("onPluginDataRegistryGetData", {
                    data: {
    ...
    DataRegistry.prototype.getLocalizationString = function (
        $formElement,
        plugin,
        key,
        callback
    ...
    DataRegistry.prototype.localizationUpdated = function (plugin, params) {
        ...
        if (params === undefined || !params.suppressLanguageEditorUpdate) {
            $.wn.builder.indexController.entityControllers.localization.languageUpdated(
                plugin
            );
        }
        $.wn.builder.indexController.entityControllers.localization.updateOnScreenStrings(
            plugin
        );
    };
...
+(function ($) {
    ...
    var EntityBase = function (typeName, indexController) {
        if (typeName === undefined) {
            throw new Error(
                "The Builder entity type name should be set in the base constructor call."
            );
        }
        if (indexController === undefined) {
            throw new Error(
                "The Builder index controller should be set when creating an entity controller."
            );
        }
    ...
    EntityBase.prototype.getSelectedPlugin = function () {
        var activeItem = $(
            "#PluginList-pluginList-plugin-list > ul > li.active"
        );
    ...
    EntityBase.prototype.updateMasterTabIdAndTitle = function (
        $tabPane,
        responseData
    ) {
...
+(function ($) {
    ...
    Plugin.prototype.cmdEditPluginSettings = function (ev) {
        ...
        $target.popup({
            handler: "onPluginLoadPopup",
            zIndex: this.popupZIndex,
            extraData: { pluginCode: $target.data("pluginCode") },
        });
    };
    ...
    Plugin.prototype.applyPluginSettingsDone = function (data) {
        if (
            ...
            data.responseData.isNewPlugin !== undefined
        ) {
            this.makePluginActive(data.responseData.pluginCode, true);
        }
    };
    Plugin.prototype.makePluginActive = function (
        pluginCode,
        updatePluginList
    ) {
        ...
        $form
            ...
            .always($.wn.builder.indexController.hideStripeIndicatorProxy)
            .done(this.proxy(this.makePluginActiveDone));
    };
    Plugin.prototype.makePluginActiveDone = function (data) {
        var pluginCode = data.responseData.pluginCode;
        $("#builder-plugin-selector-panel [data-control=filelist]").fileList(
            "markActive",
            pluginCode
        );
...
+(function ($) {
    ...
    DatabaseTable.prototype.cmdCreateTable = function (ev) {
        var result = this.indexController.openOrLoadMasterTab(
            $(ev.target),
    ...
    DatabaseTable.prototype.cmdOpenTable = function (ev) {
        var table = $(ev.currentTarget).data("id"),
            result = this.indexController.openOrLoadMasterTab(
                $(ev.target),
                "onDatabaseTableCreateOrOpen",
                this.makeTabId(table),
                { table_name: table }
            );
        if (result !== false) {
            result.done(this.proxy(this.onTableLoaded, this));
    ...
    DatabaseTable.prototype.cmdSaveMigration = function (ev) {
        ...
        $.wn.stripeLoadIndicator.show();
        $target
            .request("onDatabaseTableMigrationApply")
            .always($.wn.builder.indexController.hideStripeIndicatorProxy)
            .done(this.proxy(this.saveMigrationDone));
    };
    ...
    DatabaseTable.prototype.cmdAddTimestamps = function (ev) {
        var $target = $(ev.currentTarget),
            added = this.addTimeStampColumns($target, [
                "created_at",
                "updated_at",
            ]);
        if (!added) {
    ...
    DatabaseTable.prototype.cmdAddSoftDelete = function (ev) {
        ...
        if (!added) {
            alert(
                $target.closest("form").attr("data-lang-soft-deleting-exist")
            );
        }
    };
    DatabaseTable.prototype.onTableCellChanged = function (
        ev,
        column,
        value,
        rowIndex
    ...
    DatabaseTable.prototype.onTableLoaded = function () {
        ...
        var $masterTabPane = this.getMasterTabsActivePane(),
            ...
            $toolbar = $masterTabPane.find(
                "div[data-control=table] div.toolbar"
            ),
            $addIdButton = $(
                '<a class="btn wn-icon-hashtag builder-custom-table-button" data-builder-command="databaseTable:cmdAddIdColumn"></a>'
            ),
            $addTimestampsButton = $(
                '<a class="btn wn-icon-clock-o builder-custom-table-button" data-builder-command="databaseTable:cmdAddTimestamps"></a>'
            ),
            $addSoftDeleteButton = $(
                '<a class="btn wn-icon-refresh builder-custom-table-button" data-builder-command="databaseTable:cmdAddSoftDelete"></a>'
            );
    ...
    DatabaseTable.prototype.registerHandlers = function () {
        this.indexController.$masterTabs.on(
            "oc.tableCellChanged",
            this.proxy(this.onTableCellChanged)
        );
    };
    ...
    DatabaseTable.prototype.getTableControlObject = function ($target) {
        ...
        if (!tableObj) {
            throw new Error(
                "Table object is not found on the database table tab"
            );
        }
        return tableObj;
    };
    DatabaseTable.prototype.saveMigrationDone = function (data) {
        ...
        if (data.builderResponseData.operation != "delete") {
            $masterTabPane
                .find("input[name=table_name]")
                .val(data.builderResponseData.builderObjectName);
            this.updateMasterTabIdAndTitle(
                $masterTabPane,
                data.builderResponseData
            );
            this.unhideFormDeleteButton($masterTabPane);
            this.getTableList().fileList(
                "markActive",
                data.builderResponseData.tabId
            );
        ...
        $.wn.builder.dataRegistry.clearCache(
            data.builderResponseData.pluginCode,
            "model-columns"
        );
    };
    DatabaseTable.prototype.getTableList = function () {
        return $(
            "#layout-side-panel form[data-content-id=database] [data-control=filelist]"
        );
    };
    DatabaseTable.prototype.deleteConfirmed = function () {
        ...
        $masterTabPane
            .find("form")
            .popup({ handler: "onDatabaseTableShowDeletePopup" });
    };
    ...
    DatabaseTable.prototype.addIdColumn = function ($target) {
        ...
        if (existingColumns.indexOf("id") === -1) {
            ...
            if (
                ...
                currentData[0].unsigned ||
                currentData[0].nullable ||
                currentData[0].auto_increment ||
                currentData[0].primary_key ||
                currentData[0].default
            ) {
...
+(function ($) {
    ...
    Model.prototype.cmdDeleteModel = function (ev) {
        ...
        $.wn.confirm($target.data("confirm"), () => {
            $.request("onModelDelete", { data: { model: model }, flash: true });
        });
    };
...
+(function ($) {
    ...
    ModelForm.prototype.cmdCreateForm = function (ev) {
        var $link = $(ev.currentTarget),
            data = { model_class: $link.data("modelClass") };
    ...
    ModelForm.prototype.cmdSaveForm = function (ev) {
        var $target = $(ev.currentTarget),
            ...
            $rootContainer = $(
                "[data-root-control-wrapper] > [data-control-container]",
                $form
            ),
            $inspectorContainer = $form.find(".inspector-container"),
            controls = $.wn.builder.formbuilder.domToPropertyJson.convert(
                $rootContainer.get(0)
            );
        if (
            !$.wn.inspector.manager.applyValuesFromContainer(
                $inspectorContainer
            )
        ) {
            return;
        ...
        $target
            .request("onModelFormSave", { data: data })
            .done(this.proxy(this.saveFormDone));
    };
    ModelForm.prototype.cmdAddDatabaseFields = function (ev) {
        var $target = $(ev.currentTarget);
        var $placeholder = this.getMasterTabsActivePane().find(
            ".builder-control-list .control.placeholder:first"
        )[0];
        var fields = $target
            .find(".control-table")
            .data("oc.table")
            .dataSource.data.filter(function (column) {
                return column.add;
            })
            .reverse();
        ...
        function addField(field) {
            return function () {
                var defer = $.Deferred();
                $.wn.builder.formbuilder.controller
    ...
    ModelForm.prototype.cmdOpenForm = function (ev) {
        ...
        this.indexController.openOrLoadMasterTab(
            $(ev.target),
            "onModelFormCreateOrOpen",
            this.makeTabId(model + "-" + form),
            { file_name: form, model_class: model }
        );
    };
    ...
    ModelForm.prototype.deleteConfirmed = function () {
        ...
        $.wn.stripeLoadIndicator.show();
        $form
            .request("onModelFormDelete")
            .always($.wn.builder.indexController.hideStripeIndicatorProxy)
            .done(this.proxy(this.deleteDone));
    };
    ...
    ModelForm.prototype.getModelList = function () {
        return $(
            "#layout-side-panel form[data-content-id=models] [data-control=filelist]"
        );
    };
...
+(function ($) {
    ...
    ModelList.prototype.registerHandlers = function () {
        $(document).on(
            "autocompleteitems.oc.table",
            'form[data-sub-entity="model-list"] [data-control=table]',
            this.proxy(this.onAutocompleteItems)
        );
    };
    ...
    ModelList.prototype.cmdSaveList = function (ev) {
        ...
        $target
            .request("onModelListSave", {
                data: { columns: this.getTableData($target) },
            })
            .done(this.proxy(this.saveListDone));
    };
    ModelList.prototype.cmdOpenList = function (ev) {
        ...
        var result = this.indexController.openOrLoadMasterTab(
            $(ev.target),
            "onModelListCreateOrOpen",
            this.makeTabId(model + "-" + list),
            { file_name: list, model_class: model }
        );
        if (result !== false) {
            result.done(this.proxy(this.onListLoaded, this));
        }
    ...
    ModelList.prototype.cmdAddDatabaseColumns = function (ev) {
        ...
        $.wn.stripeLoadIndicator.show();
        $target
            .request("onModelListLoadDatabaseColumns")
            .done(this.proxy(this.databaseColumnsLoaded))
            .always($.wn.builder.indexController.hideStripeIndicatorProxy);
    };
    ...
    ModelList.prototype.loadModelFields = function (table, callback) {
        ...
        if (this.cachedModelFieldsPromises[modelClass] === undefined) {
            this.cachedModelFieldsPromises[modelClass] = $form.request(
                "onModelFormGetModelFields",
                { data: { as_plain_list: 1 } }
            );
        }
    ...
    ModelList.prototype.databaseColumnsLoaded = function (data) {
        ...
        for (var i in data.responseData.columns) {
            ...
            this.addColumn($form, column, type);
    ...
    ModelList.prototype.addColumn = function ($target, column, type) {
        var tableObj = this.getTableControlObject($target),
            currentData = this.getTableData($target),
            rowData = { field: column.name, label: column.label, type: type };
        tableObj.addRecord("bottom", true);
        tableObj.setRowValues(currentData.length - 1, rowData);
        tableObj.addRecord("bottom", false);
        tableObj.deleteRecord();
    ...
    ModelList.prototype.onListLoaded = function () {
        ...
        var $masterTabPane = this.getMasterTabsActivePane(),
            ...
            $button = $(
                '<a class="btn wn-icon-magic builder-custom-table-button" data-builder-command="modelList:cmdAddDatabaseColumns"></a>'
            );
...
+(function ($) {
    ...
    Permission.prototype.registerHandlers = function () {
        this.indexController.$masterTabs.on(
            "oc.tableNewRow",
            this.proxy(this.onTableRowCreated)
        );
    };
    ...
    Permission.prototype.cmdSavePermissions = function (ev) {
        ...
        $target
            .request("onPermissionsSave", {
                data: { permissions: this.getTableData($target) },
            })
            .done(this.proxy(this.savePermissionsDone));
    };
...
+(function ($) {
    ...
    Menus.prototype.cmdSaveMenus = function (ev) {
        ...
        $target
            .request("onMenusSave", { data: { menus: menus } })
            .done(this.proxy(this.saveMenusDone));
    };
...
+(function ($) {
    ...
    Version.prototype.cmdOpenVersion = function (ev) {
        ...
        this.indexController.openOrLoadMasterTab(
            $(ev.target),
            "onVersionCreateOrOpen",
            this.makeTabId(pluginCode + "-" + versionNumber),
    ...
    Version.prototype.cmdApplyVersion = function (ev) {
        ...
        this.showHintPopup($pane, "builder-version-apply", function () {
            $target
                .request("onVersionApply")
                .done(self.proxy(self.applyVersionDone));
        });
    };
    Version.prototype.cmdRollbackVersion = function (ev) {
        ...
        this.showHintPopup($pane, "builder-version-rollback", function () {
            $target
                .request("onVersionRollback")
                .done(self.proxy(self.rollbackVersionDone));
        });
    };
    ...
    Version.prototype.showHintPopup = function (
        $masterTabPane,
        code,
        callback
    ) {
    ...
    Version.prototype.onHintPopupHide = function (ev, $element, $modal) {
        var cbValue = $modal
                .find("input[type=checkbox][name=dont_show_again]")
                .is(":checked"),
        ...
        var $form = this.getMasterTabsActivePane().find(
            'form[data-entity="versions"]'
        );
    ...
    Version.prototype.getDontShowHintAgain = function (code, $pane) {
        ...
        return (
            $pane
                .find('input[type=hidden][data-hint-hidden="' + code + '"]')
                .val() == "true"
        );
    };
    Version.prototype.getPopupContent = function ($pane, code) {
        var template = $pane.find(
            'script[data-version-hint-template="' + code + '"]'
        );
    ...
    Version.prototype.updateUiAfterSave = function ($masterTabPane, data) {
        $masterTabPane
            .find("input[name=original_version]")
            .val(data.builderResponseData.savedVersion);
        this.updateMasterTabIdAndTitle(
            $masterTabPane,
            data.builderResponseData
        );
        this.unhideFormDeleteButton($masterTabPane);
        this.getVersionList().fileList(
            "markActive",
            data.builderResponseData.tabId
        );
        this.getIndexController().unchangeTab($masterTabPane);
    ...
    Version.prototype.getVersionList = function () {
        return $(
            "#layout-side-panel form[data-content-id=version] [data-control=filelist]"
        );
    };
    Version.prototype.updateVersionsButtons = function () {
        ...
        for (var i = $tabs.length - 1; i >= 0; i--) {
            ...
            if (isApplied) {
                $pane
                    .find('[data-builder-command="version:cmdApplyVersion"]')
                    .addClass("hide");
                $pane
                    .find('[data-builder-command="version:cmdRollbackVersion"]')
                    .removeClass("hide");
            } else {
                $pane
                    .find('[data-builder-command="version:cmdApplyVersion"]')
...
+(function ($) {
    ...
    Localization.prototype.cmdOpenLanguage = function (ev) {
        ...
        this.indexController.openOrLoadMasterTab(
            $(ev.target),
            "onLanguageCreateOrOpen",
            this.makeTabId(pluginCode + "-" + language),
    ...
    Localization.prototype.cmdSaveLanguage = function (ev) {
        var $target = $(ev.currentTarget),
            $form = $target.closest("form");
        $target
            .request("onLanguageSave")
            .done(this.proxy(this.saveLanguageDone));
    };
    ...
    Localization.prototype.cmdCopyMissingStrings = function (ev) {
        ...
        $masterTabPane
            .find("form")
            .request("onLanguageCopyStringsFrom", {
                data: { copy_from: language },
            })
            .always($.wn.builder.indexController.hideStripeIndicatorProxy)
            .done(this.proxy(this.copyStringsFromDone));
    };
    ...
    Localization.prototype.updateOnScreenStrings = function (plugin) {
        var stringElements = document.body.querySelectorAll(
            'span[data-localization-key][data-plugin="' + plugin + '"]'
        );
        $.wn.builder.dataRegistry.get(
            $("#builder-plugin-selector-panel form"),
            plugin,
            "localization",
            null,
            function (data) {
                for (var i = stringElements.length - 1; i >= 0; i--) {
                    var stringElement = stringElements[i],
                        stringKey = stringElement.getAttribute(
    ...
    Localization.prototype.saveLanguageDone = function (data) {
        ...
        var $masterTabPane = this.getMasterTabsActivePane();
        $masterTabPane
            .find("input[name=original_language]")
            .val(data.builderResponseData.language);
        this.updateMasterTabIdAndTitle(
            $masterTabPane,
            data.builderResponseData
        );
        this.unhideFormDeleteButton($masterTabPane);
        this.getLanguageList().fileList(
            "markActive",
            data.builderResponseData.tabId
        );
        this.getIndexController().unchangeTab($masterTabPane);
        if (data.builderResponseData.registryData !== undefined) {
            ...
            $.wn.builder.dataRegistry.set(
                ...
                "localization",
                null,
                registryData.strings,
                { suppressLanguageEditorUpdate: true }
            );
            $.wn.builder.dataRegistry.set(
                registryData.pluginCode,
                "localization",
                "sections",
                registryData.sections
            );
        }
    };
    Localization.prototype.getLanguageList = function () {
        return $(
            "#layout-side-panel form[data-content-id=localization] [data-control=filelist]"
        );
    };
    Localization.prototype.getCodeEditor = function ($tab) {
        return $tab
            .find("div[data-field-name=strings] div[data-control=codeeditor]")
            .data("oc.codeEditor").editor;
    };
    ...
    Localization.prototype.findDefaultLanguageForm = function (plugin) {
        var forms = document.body.querySelectorAll(
            "form[data-entity=localization]"
        );
        for (var i = forms.length - 1; i >= 0; i--) {
            var form = forms[i],
                pluginInput = form.querySelector("input[name=plugin_code]"),
                languageInput = form.querySelector(
            ...
            if (
                form.getAttribute("data-default-language") ==
                languageInput.value
            ) {
                return form;
            }
    ...
    Localization.prototype.updateLanguageFromServerDone = function (
        $languageForm,
        data
    ) {
        if (data["builderResponseData"] === undefined) {
    ...
    Localization.prototype.mergeLanguageFromServerDone = function (
        $languageForm,
        data
    ) {
        if (data["builderResponseData"] === undefined) {
...
+(function ($) {
    ...
    Controller.prototype.cmdCreateController = function (ev) {
        var $form = $(ev.currentTarget),
            ...
            behaviorsSelected = $form.find(
                'input[name="behaviors[]"]:checked'
            ).length,
            promise = null;
    ...
    Controller.prototype.cmdOpenController = function (ev) {
        ...
        this.indexController.openOrLoadMasterTab(
            $(ev.target),
            "onControllerOpen",
            this.makeTabId(pluginCode + "-" + controller),
    ...
    Controller.prototype.cmdSaveController = function (ev) {
        ...
        $target
            .request("onControllerSave")
            .done(this.proxy(this.saveControllerDone));
    };
    ...
    Controller.prototype.getControllerList = function () {
        return $(
            "#layout-side-panel form[data-content-id=controller] [data-control=filelist]"
        );
    };
...
+(function ($) {
    ...
    Builder.prototype.openOrLoadMasterTab = function (
        $form,
        serverHandlerName,
        tabId,
        data
    ) {
        if (this.masterTabsObj.goTo(tabId)) return false;
        var requestData = data === undefined ? {} : data;
        $.wn.stripeLoadIndicator.show();
        var promise = $form
            .request(serverHandlerName, { data: requestData })
    ...
    Builder.prototype.addWelcomeTab = function () {
        ...
        $.request("onWelcome")
            .done(function (data) {
                ...
                $("button[data-show-plugins]", that.$welcomeTab).click(
                    function () {
    ...
    Builder.prototype.createEntityControllers = function () {
        for (var controller in $.wn.builder.entityControllers) {
            ...
            this.entityControllers[controller] =
                new $.wn.builder.entityControllers[controller](this);
        }
    };
    Builder.prototype.registerHandlers = function () {
        ...
        this.$masterTabs.on(
            "changed.oc.changeMonitor",
            this.proxy(this.onFormChanged)
        );
        this.$masterTabs.on(
            "unchanged.oc.changeMonitor",
            this.proxy(this.onFormUnchanged)
        );
        this.$masterTabs.on("shown.bs.tab", this.proxy(this.onTabShown));
        this.$masterTabs.on(
            "afterAllClosed.oc.tab",
            this.proxy(this.onAllTabsClosed)
        );
        this.$masterTabs.on("closed.oc.tab", this.proxy(this.onTabClosed));
        this.$masterTabs.on(
            "autocompleteitems.oc.inspector",
            this.proxy(this.onDataRegistryItems)
        );
        this.$masterTabs.on(
            "dropdownoptions.oc.inspector",
            this.proxy(this.onDataRegistryItems)
        );
    ...
    Builder.prototype.addMasterTab = function (data) {
        ...
        if (data.isNewRecord) {
            ...
            $masterTabPane
                .find("form")
                .one(
                    "ready.oc.changeMonitor",
                    this.proxy(this.onChangeMonitorReady)
                );
        }
    };
    Builder.prototype.updateModifiedCounter = function () {
        ...
        $(
            "> div.tab-content > div.tab-pane[data-modified] > form",
            this.$masterTabs
        ).each(function () {
    ...
    Builder.prototype.dataToInspectorArray = function (data) {
        var result = [];
        for (var key in data) {
            var item = { title: data[key], value: key };
            result.push(item);
        }
        return result;
    };
    Builder.prototype.onCommand = function (ev) {
        ...
        var $target = $(ev.currentTarget);
        if (
            ev.currentTarget.tagName === "A" &&
            $target.attr("role") == "menuitem" &&
    ...
    Builder.prototype.onDataRegistryItems = function (ev, data) {
        var self = this;
        if (
            data.propertyDefinition.fillFrom == "model-classes" ||
            ...
            $.wn.builder.dataRegistry.get(
                $(ev.target),
                this.getFormPluginCode(ev.target),
                data.propertyDefinition.fillFrom,
                subtype,
                function (response) {
                    data.callback({
...
+(function ($) {
    ...
    LocalizationInput.prototype.init = function () {
        if (!this.options.plugin) {
            throw new Error(
                "The options.plugin value should be set in the localization input object."
            );
        }
    ...
    LocalizationInput.prototype.loadDataAndBuild = function () {
        ...
        var result = $.wn.builder.dataRegistry.get(
                this.form,
                this.options.plugin,
                "localization",
                null,
                this.proxy(this.dataLoaded)
            ),
            self = this;
        if (result) {
    ...
    LocalizationInput.prototype.loadAndShowPopup = function () {
        if (this.newStringPopupMarkup === null) {
            $.wn.stripeLoadIndicator.show();
            $(this.input)
                .request("onLanguageLoadAddStringForm")
                .done(this.proxy(this.popupMarkupLoaded))
                .always(function () {
                    $.wn.stripeLoadIndicator.hide();
                });
        } else {
    ...
    LocalizationInput.prototype.showPopup = function () {
        ...
        $.wn.builder.dataRegistry.get(
            ...
            "sections",
            function (data) {
                $keyInput.autocomplete({ source: data, matchWidth: true });
            }
        );
    ...
    LocalizationInput.prototype.stringCreated = function (data) {
        if (
            data.localizationData === undefined ||
        ...
        $.wn.builder.dataRegistry.set(
            ...
            "localization",
            null,
            data.registryData.strings
        );
        $.wn.builder.dataRegistry.set(
            this.options.plugin,
            "localization",
            "sections",
            data.registryData.sections
        );
    ...
    LocalizationInput.prototype.onSubmitPopupForm = function (ev) {
        ...
        $.wn.stripeLoadIndicator.show();
        $form
            .request("onLanguageCreateString", {
                data: { plugin_code: this.options.plugin },
            })
            .done(this.proxy(this.stringCreated))
            .always(function () {
                $.wn.stripeLoadIndicator.hide();
            });
        ev.preventDefault();
    ...
    LocalizationInput.updatePluginInputs = function (plugin) {
        var inputs = document.body.querySelectorAll(
            'input[data-builder-localization-input][data-builder-localization-plugin="' +
                plugin +
                '"]'
        );
    ...
    LocalizationInput.prototype.unregisterHandlers = function () {
        ...
        this.getContainer().off(
            "click",
            "a.localization-trigger",
            this.proxy(this.onTriggerClick)
        );
...
+(function ($) {
    ...
    LocalizationEditor.prototype.build = function () {
        ...
        editor.value = value;
        $.wn.foundation.element.addClass(
            this.containerCell,
            "text autocomplete"
        );
    ...
    LocalizationEditor.prototype.buildLocalizationEditor = function () {
        this.localizationInput = new $.wn.builder.localizationInput(
            this.getInput(),
            this.getForm(),
            {
...
+(function ($) {
    ...
    if ($.wn.table === undefined)
        throw new Error(
            "The $.wn.table namespace is not defined. Make sure that the table.js script is loaded."
        );
    if ($.wn.table.processor === undefined)
        throw new Error(
            "The $.wn.table.processor namespace is not defined. Make sure that the table.processor.base.js script is loaded."
        );
    var Base = $.wn.table.processor.string,
        BaseProto = Base.prototype;
    var LocalizationProcessor = function (
        tableObj,
        columnName,
        columnConfiguration
    ) {
    ...
    LocalizationProcessor.prototype.renderCell = function (
        value,
        cellContentContainer
    ) {
        BaseProto.renderCell.call(this, value, cellContentContainer);
    };
    LocalizationProcessor.prototype.buildEditor = function (
        cellElement,
        cellContentContainer,
        isClick
    ) {
    ...
    LocalizationProcessor.prototype.getPluginCode = function (input) {
        ...
        if (!$input.length) {
            throw new Error(
                'The input "plugin_code" should be defined in the form in order to use the localization table processor.'
            );
        }
        return $input.val();
    };
