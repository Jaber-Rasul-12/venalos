...
class IndexModelFormOperations extends IndexOperationsBehaviorBase
{
    ...
    public function onModelShowAddDatabaseFieldsPopup()
    {
        ...
        $field->value = array_map(function($field) use ($author , $plugin){
            $field['label'] = $author . '.' . $plugin . '::lang.model.' .  strtolower(Request::input('model_class')) . '.' . strtolower($field['label']); 
            return $field;
        } , $field->value);
